// Link: https://www.geeksforgeeks.org/problems/count-the-coprimes/1

#include <iostream>
#include <vector>
#include <algorithm>
using namespace std;

class Solution
{
public:
    void computeMobius(int n, vector<int> &mu)
    {
        vector<int> is_prime(n + 1, 1);
        mu[0] = 0;
        mu[1] = 1;

        for (int i = 2; i <= n; ++i)
        {
            if (is_prime[i])
            {
                for (int j = i; j <= n; j += i)
                {
                    mu[j] *= -1;
                    is_prime[j] = 0;
                }

                for (int j = i * i; j <= n; j += i * i)
                {
                    mu[j] = 0;
                }
            }
        }
    }

    void buildFre(vector<int> &arr, vector<int> &freq)
    {
        for (int x : arr)
            freq[x]++;
    }

    void computeDivCnt(int maxVal, vector<int> &freq, vector<int> &d)
    {
        for (int k = 1; k <= maxVal; ++k)
            for (int j = k; j <= maxVal; j += k)
                d[k] += freq[j];
    }

    int cntCoprime(vector<int> &arr)
    {

        int maxVal = *max_element(arr.begin(), arr.end());

        vector<int> freq(maxVal + 1, 0), d(maxVal + 1, 0), mu(maxVal + 1, 1);

        buildFre(arr, freq);
        computeDivCnt(maxVal, freq, d);
        computeMobius(maxVal, mu);

        int result = 0;
        for (int k = 1; k <= maxVal; ++k)
        {
            if (mu[k] == 0 || d[k] < 2)
                continue;

            int pairs = d[k] * (d[k] - 1) / 2;
            result += mu[k] * pairs;
        }

        return result;
    }
};

int main()
{
    Solution sol;
    vector<int> arr = {1, 2, 3};
    int result = sol.cntCoprime(arr);
    cout << "Count of coprime pairs: " << result << endl;

    arr = {4, 8, 3, 9};
    result = sol.cntCoprime(arr);
    cout << "Count of coprime pairs: " << result << endl;
    return 0;
}